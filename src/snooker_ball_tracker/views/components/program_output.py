...
class ProgramOutput(Frame):
    def __init__(self, master=None):
        ...
        self.output_log_label = Label(master=self, text="Program logs", font=self.master.master.fonts["h3-bold"])
        self.output_log_frame = Frame(master=self)
        self.output_log_scrollbar = Scrollbar(master=self.output_log_frame, orient="vertical")
        self.output_log = Text(master=self.output_log_frame, font=self.master.master.fonts["logs"], height=10, width=60, wrap="word", yscrollcommand=self.output_log_scrollbar.set)
    ...
    def grid_children(self):
        ...
        self.separator_vert.grid(column=1, row=3, rowspan=8, sticky="ns")
